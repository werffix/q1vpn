...
def delete_vpn_key_by_id(key_id: int) -> bool:
    """
    Удаляет VPN-ключ из БД.
    Перед удалением отвязывает связанные платежи и очищает notification_log.
    """
    with get_db() as conn:
        ...
        success = cursor.rowcount > 0
        if success:
            logger.info(f"Ключ ID {key_id} удалён из БД")
        return success
