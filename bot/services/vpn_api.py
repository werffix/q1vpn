...
class XUIClient:
    ...
    async def add_client(
        ...
        if expiry_time_ms and expiry_time_ms > 0:
            expire_time = int(expiry_time_ms)
        else:
        ...
        client_entry = {
            ...
            "enable": enable,
            "tgId": tg_id,
            "subId": sub_id or uuid.uuid4().hex,
            "reset": 30,
        }
    ...
    async def client_exists(self, inbound_id: int, client_uuid: str, sub_id: str = "") -> bool:
        """
        Проверяет существование клиента в inbound по UUID или subId.
        """
        inbounds = await self.get_inbounds()
        for inbound in inbounds:
            ...
            for client in settings.get("clients", []):
                cid = client.get("id") or client.get("password") or ""
                csub = client.get("subId") or ""
                if client_uuid and cid == client_uuid:
                    return True
                if sub_id and csub == sub_id:
                    return True
            return False
        return False

    async def get_client_sub_id(self, inbound_id: int, client_uuid: str) -> Optional[str]:
        """
        Возвращает subId клиента по UUID в заданном inbound.
        """
        inbounds = await self.get_inbounds()
        for inbound in inbounds:
            if inbound.get("id") != inbound_id:
    ...
    async def get_client_email(self, inbound_id: int, client_uuid: str) -> Optional[str]:
        """
        Возвращает email клиента по UUID в заданном inbound.
        """
        inbounds = await self.get_inbounds()
        for inbound in inbounds:
            if inbound.get("id") != inbound_id:
    ...
    async def get_client_stats(self, email: str, client_uuid: str = "") -> Optional[Dict[str, Any]]:
        """
        Получает статистику трафика и протокол конкретного клиента.
        
        Args:
            email: Email/идентификатор клиента
            client_uuid: UUID клиента (fallback, если email пустой)
        ...
        try:
            ...
            for inbound in inbounds:
                ...
                if client_uuid and not resolved_email:
                    settings_raw = inbound.get("settings", "{}")
                    settings = json.loads(settings_raw) if isinstance(settings_raw, str) else settings_raw
                    for c in settings.get("clients", []):
                        cid = c.get("id") or c.get("password") or ""
                        if cid == client_uuid:
                            resolved_email = c.get("email", "")
                            break

                client_stats = inbound.get("clientStats", [])
                for stats in client_stats:
                    stats_email = stats.get("email", "")
                    stats_uuid = stats.get("id") or stats.get("clientId") or stats.get("uuid") or ""
                    email_match = bool(resolved_email) and stats_email == resolved_email
                    uuid_match = bool(client_uuid) and stats_uuid == client_uuid
                    if email_match or uuid_match:
                        return {
                            "up": stats.get("up", 0),
    ...
    async def get_subscription_link(self, sub_id: str) -> Optional[str]:
        """
        Возвращает рабочий subscription URL (/sub/...).
        
        Args:
            sub_id: Subscription ID клиента
            
        Returns:
            Subscription URL или None если не удалось проверить endpoint
        """
        session = await self._ensure_session()
        ...
        for url in candidates:
            try:
                # Важно: не декодируем ответ, а возвращаем сам URL подписки.
