...
from database.requests import (
    ...
    get_active_servers, get_all_tariffs, update_vpn_key_connection,
    delete_vpn_key_by_id
)
...
from bot.services.vpn_api import (
    get_client_from_server_data, get_client, VPNAPIError, format_traffic
)
...
async def request_key_delete(callback: CallbackQuery):
    ...
    key_name = key.get('custom_name') or (key.get('client_uuid') or f"Ключ #{key_id}")
    await callback.message.edit_text(
        "⚠️ *Подтверждение удаления*\n\n"
        f"Вы действительно хотите удалить ключ:\n`{escape_md(key_name)}`\n\n"
        "Ключ будет удалён из БД, а клиент(ы) будет удалён(ы) с панелей.",
        reply_markup=key_delete_confirm_kb(key_id),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def confirm_key_delete(callback: CallbackQuery, state: FSMContext):
    ...
    key = get_vpn_key_by_id(key_id)
    if not key:
        await callback.answer("Ключ уже удалён", show_alert=True)
        return

    removed_from_panels = 0
    client_uuid = key.get('client_uuid') or ""

    # Пробуем удалить клиента с каждой активной панели и каждого inbound.
    if client_uuid:
        for server in get_active_servers():
            try:
                ...
                logger.warning(f"Не удалось подключиться к панели server_id={server['id']}: {e}")
                continue
            ...
            for inbound in inbounds:
                ...
                if inbound_id is None:
                    continue
                try:
                    exists = await client.client_exists(int(inbound_id), client_uuid, "")
                    if not exists:
                        continue
                    await client.delete_client(int(inbound_id), client_uuid)
                    removed_from_panels += 1
                except Exception as e:
                    logger.warning(
                        f"Не удалось удалить клиента uuid={client_uuid} server_id={server['id']} inbound_id={inbound_id}: {e}"
                    )

    deleted = delete_vpn_key_by_id(key_id)
    if not deleted:
        await callback.answer("❌ Не удалось удалить ключ из БД", show_alert=True)
        return

    await callback.answer(f"✅ Ключ удалён. Удалений на панелях: {removed_from_panels}", show_alert=True)
    await _show_user_view_edit(callback, state, key.get('telegram_id'))


@router.callback_query(F.data.startswith("admin_key_reissue:"))
async def reissue_key(callback: CallbackQuery, state: FSMContext):
    """Перевыпускает ключ пользователя (новый UUID/email) на том же inbound."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    inbound_id = key.get('panel_inbound_id')
    if not inbound_id:
        await callback.answer("❌ У ключа не задан inbound", show_alert=True)
        return
    ...
    # Остаток срока действия (минимум 1 день, чтобы API принял ключ)
    try:
        expires_dt = datetime.fromisoformat(str(key.get('expires_at')).replace('Z', '+00:00'))
    ...
    # Пытаемся сохранить текущий лимит трафика
    total_gb = 0
    current_email = key.get('panel_email') or ""
    current_uuid = key.get('client_uuid') or ""
    if not current_email and key.get('username'):
        current_email = f"user_{key['username']}"
    elif not current_email and key.get('telegram_id'):
        current_email = f"user_{key['telegram_id']}"

    try:
        client = get_client_from_server_data(server_data)
        stats = await client.get_client_stats(current_email, current_uuid)
        total_bytes = (stats or {}).get('total', 0)
        if total_bytes > 0:
            total_gb = max(1, int(round(total_bytes / (1024 ** 3))))

        # Старый клиент больше не нужен; если не найден — не блокируем перевыпуск
        if current_uuid:
            try:
                await client.delete_client(inbound_id, current_uuid)
            except Exception:
                pass
        ...
        new_email = generate_unique_email(user_ctx)
        ...
        logger.error(f"Ошибка перевыпуска ключа {key_id}: {e}")
        await callback.answer(f"❌ Ошибка: {e}", show_alert=True)
    except Exception as e:
        logger.error(f"Неожиданная ошибка перевыпуска ключа {key_id}: {e}")
