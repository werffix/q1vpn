...
async def _auto_setup_new_key(
    ...
    try:
        ...
        for srv in active_servers:
            ...
            if not inbounds:
                continue
            inbound_ids = [int(i['id']) for i in inbounds if i.get('id') is not None]
            chosen_inbound = inbound_id if inbound_id in inbound_ids else inbound_ids[0]

            already_exists = await client.client_exists(chosen_inbound, client_uuid, sub_id)
            created_email = ""
            if not already_exists:
                created_email = _random_email()
                flow = await client.get_inbound_flow(chosen_inbound)
                await client.add_client(
                    inbound_id=chosen_inbound,
                    email=created_email,
                    total_gb=limit_gb,
                    expire_days=days,
                    limit_ip=3,
                ...
                created_email = await client.get_client_email(chosen_inbound, client_uuid) or ""

            if srv['id'] == primary_server['id']:
                primary_inbound_id = chosen_inbound
