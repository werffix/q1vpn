...
async def _auto_setup_new_key(
    ...
    from database.requests import (
        ...
        find_order_by_order_id, get_key_details_for_user, create_initial_vpn_key, get_all_servers
    )
    from bot.services.vpn_api import get_client
    ...
    try:
        ...
        common_expiry_ms = int((time.time() + days * 86400) * 1000) if days > 0 else 0

        active_servers = get_all_servers()
        ...
        sub_id = ""
        if existing_key and existing_key.get('server_id') and existing_key.get('panel_inbound_id') and existing_key.get('client_uuid'):
            try:
                existing_client = await get_client(existing_key['server_id'])
                sub_id = await existing_client.get_client_sub_id(existing_key['panel_inbound_id'], existing_key['client_uuid']) or ""
            except Exception:
                sub_id = ""
        if not sub_id:
        ...
        primary_server = next(
            (s for s in active_servers if "германия" in str(s.get('name', '')).lower()),
            next((s for s in active_servers if not _is_ip_host(s.get('host', ''))), active_servers[0])
        )
        primary_inbound_id = None
        primary_email = ""
        fallback_server_id = None
        fallback_inbound_id = None
        fallback_email = ""
        processed_any_inbound = False

        for srv in active_servers:
            try:
                client = await get_client(srv['id'])
                inbounds = await client.get_inbounds()
            except Exception as e:
                logger.error(f"Панель недоступна server_id={srv['id']}: {e}")
                continue
            if not inbounds:
                logger.warning(f"На панели нет inbound-ов server_id={srv['id']}")
                continue
            inbound_ids = sorted({int(i['id']) for i in inbounds if i.get('id') is not None})
            for current_inbound_id in inbound_ids:
                try:
                    already_exists = await client.client_exists(current_inbound_id, client_uuid, sub_id)
                    created_email = ""
                    if not already_exists:
                        created_email = _random_email()
                        flow = await client.get_inbound_flow(current_inbound_id)
                        await client.add_client(
                            inbound_id=current_inbound_id,
                            email=created_email,
                            total_gb=limit_gb,
                            expire_days=days,
                            expiry_time_ms=common_expiry_ms,
                            limit_ip=3,
                            enable=True,
                        ...
                        created_email = await client.get_client_email(current_inbound_id, client_uuid) or ""
                        logger.info(
                            f"Клиент уже существует, пропуск: server_id={srv['id']} inbound_id={current_inbound_id} uuid={client_uuid}"
                        )

                    processed_any_inbound = True

                    if fallback_inbound_id is None:
                        fallback_server_id = srv['id']
                        fallback_inbound_id = current_inbound_id
                        fallback_email = created_email

                    if srv['id'] == primary_server['id'] and primary_inbound_id is None:
                        primary_inbound_id = current_inbound_id
                        primary_email = created_email
                except Exception as e:
                    logger.error(
                        f"Ошибка создания клиента server_id={srv['id']} inbound_id={current_inbound_id}: {e}"
                    )
                    continue

        if primary_inbound_id is None and fallback_inbound_id is not None:
            primary_server = {'id': fallback_server_id}
            primary_inbound_id = fallback_inbound_id
            primary_email = fallback_email

        if not processed_any_inbound or primary_inbound_id is None:
            raise ValueError("Не удалось создать клиента ни на одном сервере")

        update_vpn_key_config(
            key_id=key_id,
            server_id=primary_server['id'],
            panel_inbound_id=primary_inbound_id,
            panel_email=primary_email,
            client_uuid=client_uuid
        )
...
async def process_new_key_final(callback: CallbackQuery, state: FSMContext, server_id: int, inbound_id: int):
    ...
    from database.requests import (
        ...
        get_key_details_for_user, create_initial_vpn_key, get_all_servers
    )
    from bot.services.vpn_api import get_client
    from bot.utils.key_sender import send_key_with_qr
    from bot.keyboards.user import key_issued_kb
