...
MAIN_TRAFFIC_SERVER_IDS = [3]
WHITELIST_TRAFFIC_SERVER_IDS = [2, 4]
...
async def _get_group_traffic(telegram_id: int, server_ids: List[int]) -> Dict[str, Any]:
    ...
    for key in keys:
        if key.get('server_id') not in server_ids:
            continue
        try:
            client = await get_client(key['server_id'])
            stats = await client.get_client_stats(key.get('panel_email', ''), key.get('client_uuid', ''))
...
async def show_cabinet(telegram_id: int, send_function):
    ...
    main_traffic = await _get_group_traffic(telegram_id, MAIN_TRAFFIC_SERVER_IDS)
    whitelist_traffic = await _get_group_traffic(telegram_id, WHITELIST_TRAFFIC_SERVER_IDS)
...
async def connect_android_handler(callback: CallbackQuery):
    try:
        sub_link = await _get_active_key_link(callback.from_user.id)
        if sub_link:
            await _send_subscription_qr(callback.message, sub_link)
