...
async def _get_active_key_link(telegram_id: int) -> Optional[str]:
    ...
    for key in keys:
        ...
        try:
            ...
            return f"{protocol}://{key['host']}:{key['port']}{path_prefix}/sub/{sub_id}"
        except Exception as e:
...
async def show_connect_android(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        f"2. Отсканируйте QR-код выше или вставьте ключ подписки:\n`{key_text}`\n"
...
async def show_connect_windows(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        f"2. Откройте Telegram на ПК, отсканируйте QR-код выше или вставьте ключ подписки в Happ:\n`{key_text}`\n\n"
        "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!",
