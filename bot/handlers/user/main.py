...
async def _get_active_key_link(telegram_id: int) -> Optional[str]:
    ...
    for key in keys:
        ...
        try:
            ...
            host_part = f"{host}:{SUBSCRIPTION_PUBLIC_PORT}"
            return f"{protocol}://{host_part}/sub/{sub_id}"
        except Exception as e:
    ...
    for key in keys:
        ...
        try:
            ...
            if sub_id and key.get('host') and key.get('port'):
                protocol = key.get('protocol') or 'https'
                host_part = f"{key['host']}:{SUBSCRIPTION_PUBLIC_PORT}"
                sub_urls.append(f"{protocol}://{host_part}/sub/{sub_id}")
