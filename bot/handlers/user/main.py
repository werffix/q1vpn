...
async def _get_primary_key_traffic(telegram_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        stats = await client.get_client_stats(key.get('panel_email', ''), key.get('client_uuid', ''))
        if not stats:
            return info
...
async def _get_group_traffic(telegram_id: int, server_ids: List[int]) -> Dict[str, Any]:
    """Суммарный трафик группы серверов по ключам пользователя."""
    from database.requests import get_active_servers, get_user_primary_key_for_profile
    from bot.services.vpn_api import get_client, format_traffic

    used_bytes = 0
    total_bytes = 0
    key = get_user_primary_key_for_profile(telegram_id)
    if not key or not key.get('client_uuid'):
        return {
            'used_bytes': 0,
            'total_bytes': 0,
            'used_human': "0 GB",
    ...
    for server in get_active_servers():
        if server.get('id') not in target_ids:
            continue
        try:
            client = await get_client(server['id'])
            email_hint = key.get('panel_email', '') if server['id'] == key.get('server_id') else ""
            stats = await client.get_client_stats(email_hint, key.get('client_uuid', ''))
            if not stats:
                continue
            used_bytes += stats.get('up', 0) + stats.get('down', 0)
            total_bytes += stats.get('total', 0)
    ...
    return {
        ...
        'used_human': format_traffic(used_bytes),
        'total_human': format_traffic(total_bytes) if total_bytes > 0 else "Безлимит",
    }
...
async def _get_whitelist_traffic(telegram_id: int) -> Dict[str, Any]:
    """Трафик по серверам, где домен задан как IP-адрес."""
    from database.requests import get_active_servers
    server_ids = {
        int(server['id'])
        for server in get_active_servers()
        if server.get('id') and _is_ip_host(server.get('host', ''))
    }
    if not server_ids:
        return {
            'used_bytes': 0,
    ...
    return await _get_group_traffic(telegram_id, list(server_ids))
...
async def _get_active_key_link(telegram_id: int) -> Optional[str]:
    """Возвращает subscription URL активного ключа пользователя."""
    details = await _get_active_key_link_details(telegram_id)
    return details.get("subscription_url")


def _is_germany_server_name(name: str) -> bool:
    lowered = (name or "").strip().lower()
    return "германия" in lowered or "germany" in lowered


async def _get_active_key_link_details(telegram_id: int) -> Dict[str, Any]:
    """SUB-ссылка + VLESS из остальных серверов (кроме Германии)."""
    from database.requests import get_user_keys_for_subscription
    from bot.services.vpn_api import get_client
    from bot.utils.key_sender import build_subscription_link
    from bot.utils.key_generator import generate_link

    keys = get_user_keys_for_subscription(telegram_id)
    if not keys:
        return {"subscription_url": None, "extra_vless_links": []}

    germany_first = sorted(
        keys,
        key=lambda k: 0 if _is_germany_server_name(str(k.get("server_name") or "")) else 1
    )

    base_sub_url: Optional[str] = None
    base_sub_id: str = ""
    base_client_uuid: str = ""
    for key in germany_first:
        if not key.get('server_id') or not key.get('client_uuid'):
            continue
        if not _is_not_expired(key.get('expires_at')):
            continue
        try:
            client = await get_client(key['server_id'])
            inbound_id = key.get('panel_inbound_id')
            sub_id = await client.get_client_sub_id(inbound_id, key['client_uuid']) if inbound_id else None
            if not sub_id:
                continue
            protocol = key.get('protocol') or 'https'
            host = str(key.get('host') or '').strip()
            base_sub_url = build_subscription_link(sub_id, protocol, host)
            base_sub_id = sub_id
            base_client_uuid = key.get('client_uuid', '')
            break
        except Exception as e:
            logger.warning(f"Не удалось получить subscription URL user={telegram_id}: {e}")
            continue
    ...
    for srv in get_all_servers():
        if not srv.get("is_active"):
            continue
        if _is_germany_server_name(str(srv.get("name") or "")):
            continue

        try:
            client = await get_client(int(srv["id"]))
            ...
            for inbound in inbounds:
                ...
                exists = await client.client_exists(int(inbound_id), base_client_uuid, base_sub_id)
                if not exists:
                    continue

                email = await client.get_client_email(int(inbound_id), base_client_uuid)
                if not email:
                    continue

                config = await client.get_client_config(email)
                if not config or str(config.get("protocol", "")).lower() != "vless":
                    continue
            ...
            logger.warning(f"Не удалось получить доп. VLESS server_id={srv.get('id')}: {e}")
            continue

    return {"subscription_url": base_sub_url, "extra_vless_links": extra_links}


async def _send_subscription_qr(message: Message, subscription_url: str) -> None:
    """Отправляет QR-код для subscription URL."""
    try:
        import qrcode
        ...
        await message.answer_photo(
            BufferedInputFile(image_bytes.getvalue(), filename="subscription_qr.png"),
            caption="📷 QR-код подписки"
        )
    except Exception as e:
        logger.warning(f"Не удалось отправить QR подписки: {e}")
...
async def activate_trial_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    from database.requests import (
        ...
        create_pending_order, complete_order, is_user_referred
    )
    ...
    if is_user_referred(internal_user_id) and duration_days < 14:
        duration_days = 14
...
async def show_cabinet(telegram_id: int, send_function):
    ...
    main_traffic = await _get_group_traffic(telegram_id, [MAIN_TRAFFIC_SERVER_ID])
    whitelist_traffic = await _get_whitelist_traffic(telegram_id)
    ...
    await send_function(
        ...
        "🛩 Использование трафика:\n"
        f"Основной: {main_traffic['used_human']} из {main_traffic['total_human']}\n"
        f"Белые списки: {whitelist_traffic['used_human']} из {whitelist_traffic['total_human']}",
        reply_markup=cabinet_kb()
    )
...
async def show_referrals(telegram_id: int, bot, send_function):
    ...
    text = (
        ...
        "🗣 За каждого приведенного пользователя Вы получите 7 дней подписки, "
        "а Ваш друг - пробный период в 14 дней\n\n"
...
async def show_connect_android(telegram_id: int, send_function):
    ...
    key_details = await _get_active_key_link_details(telegram_id)
    key_link = key_details.get("subscription_url")
    key_text = key_link or "Ссылка ещё подготавливается, попробуйте снова через несколько секунд."
    extra_links = key_details.get("extra_vless_links", [])
    extra_block = ""
    if extra_links:
        extra_block = "\n\nДополнительные VLESS (кроме Германии):\n" + "\n".join(
    ...
    await send_function(
        ...
        f"2. Вставьте ссылку подписки в приложение:\n`{key_text}`{extra_block}\n"
...
async def show_connect_windows(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        f"2. Вставьте ссылку подписки в Happ:\n`{key_text}`{extra_block}\n\n"
        "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!",
    ...
    for key in keys:
        ...
        try:
            ...
            if sub_id and key.get('host') and key.get('port'):
                protocol = key.get('protocol') or 'https'
                sub_urls.append(build_subscription_link(sub_id, protocol, str(key.get('host') or '').strip()))
