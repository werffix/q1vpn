...
async def _get_active_key_link(telegram_id: int) -> Optional[str]:
    ...
    for key in keys:
        ...
        try:
            ...
            port = int(key.get('port') or 0)
            omit_port = (protocol == 'https' and port == 443) or (protocol == 'http' and port == 80)
            host_part = host if omit_port else f"{host}:{port}"
            return f"{protocol}://{host_part}{path_prefix}/sub/{sub_id}"
        except Exception as e:
...
async def show_connect_android(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        "2. Нажмите на «🔗 Активировать VPN-профиль», чтобы добавить подключение в приложение.\n"
        f"Если кнопка не сработала — вставьте ссылку вручную:\n`{key_text}`\n"
        "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!",
        reply_markup=connect_android_kb(key_link),
        parse_mode="Markdown"
    )
...
async def show_connect_ios(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        f"Если кнопка не сработала — вставьте ссылку вручную:\n`{key_text}`\n\n"
        "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!",
        reply_markup=connect_ios_kb(key_link),
        parse_mode="Markdown"
    )
...
async def show_connect_windows(telegram_id: int, send_function):
    ...
    await send_function(
        ...
        f"Если кнопка не сработала — вставьте ссылку вручную:\n`{key_text}`\n\n"
        "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!",
        reply_markup=connect_windows_kb(key_link),
        parse_mode="Markdown"
    )
    ...
    for key in keys:
        ...
        try:
            ...
            if sub_id and key.get('host') and key.get('port'):
                ...
                port = int(key['port'])
                omit_port = (protocol == 'https' and port == 443) or (protocol == 'http' and port == 80)
                host_part = key['host'] if omit_port else f"{key['host']}:{port}"
                sub_urls.append(f"{protocol}://{host_part}{path_prefix}/sub/{sub_id}")
